# Monica Steam 首个公开测试版本

Monica Steam 是一款专注于 Steam 账号管理与日常使用的第三方 Android 客户端。本版本从 Steam 令牌管理出发，加入游戏库、商店、好友聊天、通知和数据备份等功能。

> [!WARNING]
> 本次发布是用于公开测试的早期版本，并非正式版或稳定版。软件仍在持续开发，部分功能可能不完整、失效或出现闪退，请勿将其作为唯一的 Steam 令牌及账号数据存储方式。使用前请务必备份现有 `maFile` 和其他重要数据。
...
这是 Monica Steam 的第一个公开测试版本，并非正式发布版本。当前版本主要用于收集兼容性、稳定性和功能反馈，后续更新可能调整功能、界面及本地数据结构。

部分功能依赖 Steam 的非公开接口和网页服务，Steam 更新接口、登录策略或地区规则后，相关功能可能需要同步适配。
