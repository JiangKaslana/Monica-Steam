/*
 * Adapted from Kyant0/AndroidLiquidGlass and the compose-miuix-ui example
 * used by KernelSU. Both upstream implementations are Apache-2.0.
 */
...
internal fun BackdropEffectScope.liquidGlassLens(
    ...
    val key = if (dispersionEnabled) {
        "MonicaSteamLiquidGlassLensDispersion"
    } else {
        "MonicaSteamLiquidGlassLens"
    }

    val scaleFactor = downscaleFactor.coerceAtLeast(1).toFloat()
    val scaledSizeWidth = size.width / scaleFactor
    val scaledSizeHeight = size.height / scaleFactor
    ...
    runtimeShaderEffect(
        ...
        setFloatUniform("size", scaledSizeWidth, scaledSizeHeight)
