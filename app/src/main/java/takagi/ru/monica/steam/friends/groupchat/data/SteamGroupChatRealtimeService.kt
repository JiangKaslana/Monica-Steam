...
internal interface SteamGroupChatRealtimeTransport {
...
internal class SteamCmGroupChatRealtimeTransport(
...
internal class SteamGroupChatRealtimeService(
    private val transport: SteamGroupChatRealtimeTransport =
        SteamCmGroupChatRealtimeTransport(SteamCmClient()),
    ...
    internal constructor(
        ...
        transport = SteamCmGroupChatRealtimeTransport(cm),
        sessionResolver = sessionResolver
    )

    override fun events(account: SteamAccount): Flow<SteamGroupChatRealtimeEvent> = channelFlow {
        var sessionAccount = account
        val eventCollector = launch {
            // CM event routing uses the immutable account/storage key, not the
            // rotating access token, so this collector survives token refresh.
            transport.events(account).collect { envelope ->
                runCatching {
                    SteamGroupChatRealtimeParser.parse(envelope)
        ...
        try {
            while (isActive) {
                ...
                if (connected != announcedConnected) {
                    ...
                    send(SteamGroupChatRealtimeEvent.ConnectionChanged(connected))
                }
                if (connected) {
